...
( function( $ ) {
	...
	$.fn.baseSwipe = function( option ) {
		...
		return this.each( function() {
			$( this ).data( 'baseSwipe', new baseSwipe( option ) );
		});
	};
	 ...
	 */
	$.fn.baseSwipe2start = function() {
		return this.each( function() {
			$( this ).data( 'baseSwipe' ).startSlideShow();
		});
	};
	 ...
	 */
	$.fn.baseSwipe2stop = function() {
		return this.each( function() {
			$( this ).data( 'baseSwipe' ).stopSlideShow();
		});
	};
	 ...
	 */
	$.fn.baseSwipe2prev = function() {
		return this.each( function() {
			$( this ).data( 'baseSwipe' ).toPrev();
		});
	};
	 ...
	 */
	$.fn.baseSwipe2next = function() {
		return this.each( function() {
			$( this ).data( 'baseSwipe' ).toNext();
		});
	};
	 ...
	 */
	$.fn.baseSwipe2slide = function( _idx ) {
		return this.each( function() {
			$( this ).data( 'baseSwipe' ).toSlide( _idx );
		});
	};
	 ...
	 * @method: 슬라이드 제거
	 */       
	$.fn.baseSwipe2destory = function( _idx ) {
		return this.each( function() {
			$( this ).data( 'baseSwipe' ).destory();
		});
	};
}( jQuery ) );
...
function baseSwipe( __setting ) {
	...
	var helper = { // 보조함수
		...
		getCss3AnimateTranslate: function( _name, _from_pos, _to_pos ) {
			...
			css_txt +=	'from {';
			css_txt +=		browser_Prefix.transformsCss + ': translateX(' + _from_pos + '%);';
			css_txt +=	'}';
			css_txt +=	'to {';
			css_txt +=		browser_Prefix.transformsCss + ': translateX(' + _to_pos + '%);';
			css_txt +=	'}';
			css_txt += '}';
	...
	function setInitStyle() {
		...
		for ( i = 0, len = list_Len; i < len; i++ ) {
			pos = setting.loop && list_Len - 1 === i ? -100 : 100 * i;
			...
			css_txt += 'width: 70%; ';
			css_txt += 'height: 100%; ';
			css_txt += 'padding-left: 5%; ';
			css_txt += 'padding-right: 5%; ';
			D_List[ i ].style.cssText = css_txt;
			helper.setCss3Transition( D_List[ i ], 0, list_Pos_Arr[ i ] );
		}

		// 페인트 타임으로 인하여, css3의 animate로 접근해야 함
		css_txt = helper.getCss3AnimateTranslate( 'next-to-now', 100, 0 );
		css_txt += helper.getCss3AnimateTranslate( 'now-to-prev', 0, -100 );
		css_txt += helper.getCss3AnimateTranslate( 'prev-to-now', -100, 0 );
		css_txt += helper.getCss3AnimateTranslate( 'now-to-next', 0, 100 );

		css_dom = document.createElement( 'style' );
		css_dom.setAttribute( 'type', 'text/css' );
		css_dom.setAttribute( 'data-swipe', 'baseSwipe' );
	...
	function toSlideAnimate( _time, _way ) {
		...
		if ( now_pos === 0 ) {
			...
			// touch로 접근시
			if ( now_pos % 100 === 0 ) { // 사용자가 빠르게 터치해서 이미 끝으로 도달 했을 시
				toSlideAnimateAfter({
					target: D_List[ now_idx ]
				});
			} else {
				helper.setCss3Transition( D_List[ now_idx ], _time, _way === 'next' ? -100 : 100 );
	...
	function toSlideAnimateAfter( e ) {
		...
		list_Pos_Arr[ now_idx ] = now_idx < to_idx ? -100 : 100;
